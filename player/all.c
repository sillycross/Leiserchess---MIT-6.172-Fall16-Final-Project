...
static inline void initialize_pv_node(searchNode* node, int depth) {
  node->type = SEARCH_PV;
...
// -----------------------------------------------------------------------------
static inline void initialize_root_node(searchNode *node, score_t alpha, score_t beta, int depth,
...
static inline move_t tt_move_of(ttRec_t *rec) {
  return rec->move;
}

// getting the score out of the record
static inline score_t tt_score_of(ttRec_t *rec) {
  return rec->score;
}
...
static inline void tt_resize_hashtable(int size_in_meg) {
...
static inline void tt_make_hashtable(int size_in_meg) {
...
static inline void tt_clear_hashtable() {
  memset(hashtable.tt_set, 0, sizeof(ttSet_t) * hashtable.num_of_sets);
  hashtable.age = 0;
}
...
static inline score_t win_in(int ply)  {
  return  WIN - ply;
}

static inline score_t lose_in(int ply) {
  return -WIN + ply;
}
...
static inline score_t tt_adjust_score_from_hashtable(ttRec_t *rec, int ply_in_search) {
...
static inline score_t tt_adjust_score_for_hashtable(score_t score, int ply_in_search) {
  if (score >= win_in(MAX_PLY_IN_SEARCH)) {
    return score + ply_in_search;
  }
  if (score <= lose_in(MAX_PLY_IN_SEARCH)) {
